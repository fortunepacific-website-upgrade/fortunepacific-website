<?php !isset($c) && exit();?>
<?php
manage::check_permit('seo.article', 2);//检查权限

$type=(int)$_GET['type'];
$result=manage::ueeseo_get_data('ueeshop_ueeseo_get_article', array('type'=>$type));
!$result['ret'] && $result['msg']=array();

$count_result=manage::ueeseo_get_data('ueeshop_ueeseo_get_article', array('type'=>0));
$Total_task = count($count_result['msg']['intro_row']);
$ThisWeekComplete_num = $Total_task-count($count_result['msg']['article_row']);
...
<script language="javascript">$(document).ready(function(){seo_obj.article_init();});</script>
<div id="article" class="r_con_wrap">
	<div class="inside_container">
		<h1>{/module.seo.article/}</h1>
	</div>
	...
	<div class="global_container">
        <div class="cCards">
        	<ul>
                <li class="<?=$type==0?'on':'';?>"><a href="./?m=seo&a=article&type=0">{/seo.complete_ary.0/}</a></li>
                <li class="<?=$type==1?'on':'';?>"><a href="./?m=seo&a=article&type=1">{/seo.complete_ary.1/}</a></li>
            </ul>
            <div class="clear"></div>
        </div>
        <div class="cLists">
        	<ul class="r_con_list">
				<?php
				foreach((array)$result['msg']['article_row'] as $k=>$v){
				?>
                    <li data="<?=$v['ArticleId'];?>">
                        <dl>
