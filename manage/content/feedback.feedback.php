...
$(document).ready(function(){
	feedback_obj.feedback_init();
});
</script>
<div class="r_nav feedback_nav">
	<h1>{/feedback.feedback_name/}</h1>
    <?php
	if($d=='list'){
		$where='1';//条件
		$page_count=20;//显示数量
    ...
    <div class="list_nav fr">
        ...
        <div id="turn_page_oth" class="turn_page fl"><?=manage::turn_page($feedback_row[1], $feedback_row[2], $feedback_row[3], '?'.ly200::query_string('page').'&page=', '{/global.pre_page/}', '{/global.next_page/}');?></div>
    </div>
    <?php }?>
</div>
            ...
            <table border="0" cellpadding="5" cellspacing="0" class="r_con_table">
                ...
                <tbody>
                    ...
                    <tr class="<?=(int)$v['IsRead']?'':'fc_red';?>">
                        ...
                        <td class="last">
                            <a href="./?m=content&a=feedback.feedback&d=edit&FId=<?=$fid;?>" title="{/global.view/}"><img src="/static/images/ico/search.png" alt="{/global.view/}" align="absmiddle" /></a>
                            <a href="./?do_action=content.feedback_del&FId=<?=$fid;?>" title="{/global.del/}" class="del" rel="del"><img src="/static/images/ico/del.png" alt="{/global.del/}" align="absmiddle" /></a>
                        </td>
                    </tr>
                    <?php }?>
                </tbody>
            </table>
            <input type="hidden" name="do_action" value="content.feedback_explode" />
            <input type="hidden" name="IdStr" value="" />
            <input type="hidden" name="Type" value="feedback" />
        </form>
        <div class="blank20"></div>
        <div id="turn_page_oth"><?=manage::turn_page($feedback_row[1], $feedback_row[2], $feedback_row[3], '?'.ly200::query_string('page').'&page=', '{/global.pre_page/}', '{/global.next_page/}');?></div>
        <div class="blank20"></div>
    </div>
	<?php
		//销毁变量
		...
		!$feedback_row && js::location('./m=content&a=feedback.feedback');
		
		db::update('feedback', "FId='$FId'", array('IsRead'=>1));
		$email=urlencode($feedback_row['Email'].'/'.$feedback_row['Name']);
	?>
	<div class="edit_bd list_box">
		<form id="feedback_form" name="feedback_form" class="r_con_form">
			<div class="rows_box">
				<div class="rows">
					<label>{/products.inquiry.title/}:</label>
					<span class="input"><?=$feedback_row['Subject'];?></span>
					<div class="clear"></div>
				</div>
				<div class="rows">
					<label>{/products.inquiry.customer_name/}:</label>
					<span class="input"><?=$feedback_row['Name'];?></span>
					<div class="clear"></div>
				</div>
				<div class="rows">
					<label>{/feedback.company/}:</label>
					<span class="input"><?=$feedback_row['Company'];?></span>
					<div class="clear"></div>
				</div>
				<div class="rows">
					<label>{/products.inquiry.mail/}:</label>
					<span class="input"><?=$feedback_row['Email'];?><a href="./?m=email&a=send&Email=<?=$email;?>" class="blue mar_l_10" target="_blank">{/products.inquiry.mail_reply/}</a></span>
					<div class="clear"></div>
				</div>
				<div class="rows">
					<label>{/products.inquiry.tel/}:</label>
					<span class="input"><?=$feedback_row['Phone'];?></span>
					<div class="clear"></div>
				</div>
				<div class="rows">
					<label>{/feedback.mobile/}:</label>
					<span class="input"><?=$feedback_row['Mobile'];?></span>
					<div class="clear"></div>
				</div>
				<div class="rows">
					<label>{/manage.log.ip/}:</label>
					<span class="input"><?=$feedback_row['Ip'].' 【'.ly200::ip($feedback_row['Ip']).'】';?></span>
					<div class="clear"></div>
				</div>
				<div class="rows">
					<label>{/global.time/}:</label>
					<span class="input"><?=date('Y/m/d H:i:s', $feedback_row['AccTime']);?></span>
					<div class="clear"></div>
				</div>
				<div class="rows">
					<label>{/feedback.content/}:</label>
					<span class="input"><?=str::format($feedback_row['Message']);?></span>
					<div class="clear"></div>
				</div>
				
				<div class="rows">
					<label></label>
					<span class="input"><a href="./?m=content&a=feedback.feedback" class="btn_cancel">{/global.return/}</a></span>
					<div class="clear"></div>
				</div>
			</div>
		</form>
