...
class VerCode{
    ...
    var $_x			= 0;
    var $_y			= 0;
    var $_fault		= 3;//容错象素 越大体验越好，越小破解难道越高
    ...
    function check($offset=''){
        if(!$_SESSION['v_code_r']){
            return false;
        }
        if(!$offset){
            $offset=$_REQUEST['vr'];
        }
        $ret=abs($_SESSION['v_code_r']-$offset)<=$this->_fault;
        if($ret){
            unset($_SESSION['v_code_r']);
        }else{
            $_SESSION['v_code_err']++;
            if($_SESSION['v_code_err']>3){//错误3次必须刷新
                unset($_SESSION['v_code_r'], $_SESSION['Ueeshop']['VerCode']);
            }
        }
        return $ret;
    }
    ...
    private function _init(){
        ...
        $_SESSION['v_code_r']=$this->_x=mt_rand(50, $this->bg_width-$this->mark_width-1);
        $_SESSION['v_code_err']=0;
		$_SESSION['Ueeshop']['VerCode']=0;
    ...
    private function _createBg(){
        $file_mark=dirname(__FILE__).'/images/mark.png';
    ...
    private function _createSlide(){
        $file_mark=dirname(__FILE__).'/images/mark2.png';
