...
*/
    ...
    $this->_tempdir = $c['root_path'].$c['tmp_dir'];
}
...
## ABR
    if ($this->_tmpfilename != '') {
        $OLE  = new writeexcel_olewriter($c['tmp_dir'].$this->_tmpfilename);
        $OLE->_OLEtmpfilename = $c['tmp_dir'].$this->_tmpfilename;
    } else {
...
function _store_all_fonts() {
    ...
    for ($i=0;$i<sizeof($this->_formats);$i++) {
